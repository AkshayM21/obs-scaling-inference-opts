...
@register_model("hf_reason", "huggingface_reason")
class HFLM_Reasoning(TemplateLM):
    """
    ...
    def loglikelihood_rolling(self, requests: List[Instance], disable_tqdm: bool = False ) -> List[float]:
        ...
        generation_params = {
            ...
            "do_sample": True,
            "temperature": 0.7,
            "max_gen_toks": 100  # Keep reasonable max token limit
        }
        
        # Create generate_until requests from the original requests
        generate_requests = []
        for req in requests:
            string = req.args[0]
            generate_requests.append(Instance(args=(string, generation_params)))
        
        # Get model generations
        generated_responses = self.generate_until(generate_requests, disable_tqdm=disable_tqdm)
        
        # For each response, append "The answer is: " and evaluate likelihood
        augmented_requests = []
        for req_idx, (req, response) in enumerate(zip(requests, generated_responses)):
            original_string = req.args[0]
            augmented_string = response + " The answer is: "
            augmented_requests.append(Instance(args=(augmented_string,)))
